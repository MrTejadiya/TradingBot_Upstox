...
void migrations_are_ordered_and_named() {
    ...
    require(migrations.size() == 3, "three baseline migrations should exist");
...
void pending_migrations_skip_applied_versions() {
    ...
    require(pending.size() == 2, "two migrations should remain after version 1");
...
void applies_pending_migrations_idempotently() {
    ...
    require(first_count == 3, "first apply should apply all migrations");
    require(second_count == 0, "second apply should be idempotent");
    require(store.applied_migrations().size() == 3, "store should retain applied migrations");
}
