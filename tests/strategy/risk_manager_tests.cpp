...
void approves_order_value_within_daily_limit() {
    ...
    request.max_daily_traded_value = 5000.0;

    const auto event = tradingbot::strategy::RiskManager{}.evaluate(request);

    require(event.decision == tradingbot::core::RiskDecision::Approved, "daily traded value below limit should approve");
}

void rejects_projected_daily_traded_value_above_limit() {
    auto request = base_request();
    request.traded_value_today = 4800.0;
    request.max_daily_traded_value = 5000.0;

    const auto event = tradingbot::strategy::RiskManager{}.evaluate(request);

    require(event.reason_code == "MAX_DAILY_TRADED_VALUE_EXCEEDED",
            "projected daily traded value above limit should reject");
}
